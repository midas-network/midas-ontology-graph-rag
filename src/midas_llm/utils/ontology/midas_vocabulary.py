"""
Parse MIDAS OWL ontology into constrained vocabularies for LLM extraction.

Pipeline:
  1. Parse OWL/RDF → extract classes with labels, alt terms, definitions, hierarchy
  2. Map ontology classes to extraction schema fields
...
@dataclass
class OntologyClass:
    """A single class from the MIDAS OWL ontology."""
    iri: str
    label: str = ""
...
# ═══════════════════════════════════════════════════════════════════════
# Field mapping: which ontology classes map to which schema fields
# ═══════════════════════════════════════════════════════════════════════
...
class MIDASVocabulary:
    """Controlled vocabulary extracted from the MIDAS OWL ontology."""
    ...
    def _build_field_mapping(self):
        """Map each ontology class to an extraction field."""
    ...
    def build_json_schema(self) -> dict:
        ...
        Fields with MIDAS ontology terms use enum+additionalItems
        so the LLM prefers ontology terms but can add novel ones.
        """
        # Fixed categorical vocabularies (not from ontology)
        fixed_enums = {
            "model_determinism": [
        ...
        # MIDAS ontology-sourced fields → enum with free-text fallback
                ...
                properties[field_name] = {
                    ...
                    "properties": {
                        "values": {
                            ...
                            "description": (
                                f"Prefer these MIDAS ontology terms: "
    ...
    def build_prompt_section(self) -> str:
        """Build a prompt section listing preferred ontology terms."""
        lines = [
            "",
            "═══════════════════════════════════════════════",
            ...
            "PREFER these exact terms from the MIDAS ontology.",
            "Only use free text if no ontology term matches.",
            "",
        ]
